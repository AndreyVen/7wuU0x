#!/usr/bin/env bash
# Copyright (c) 2016 The Vendetta Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

###   This script attempts to download the signature file SHA256SUMS.asc from
###   vendettacore.org and vendetta.org and compares them.
...
WORKINGDIR="/tmp/vendetta_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://vendettacore.org"
HOST2="https://vendetta.org"
BASEDIR="/bin/"
VERSIONPREFIX="vendetta-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'vendetta-',
...
if [ $? -ne 0 ]; then
   echo "vendetta.org failed to provide signature file, but vendettacore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "vendetta.org and vendettacore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Vendetta Core binary release signing key installed?"
   fi
