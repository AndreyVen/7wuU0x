...
pkgdatadir = $(datadir)/vendetta
pkgincludedir = $(includedir)/vendetta
pkglibdir = $(libdir)/vendetta
pkglibexecdir = $(libexecdir)/vendetta
...
CONFIG_HEADER = $(top_builddir)/src/config/vendetta-config.h
CONFIG_CLEAN_FILES = libvendettaconsensus.pc share/setup.nsi \
...
CONFIG_CLEAN_VPATH_FILES = contrib/filter-lcov.py \
	test/functional/test_runner.py test/util/vendetta-util-test.py \
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	$(srcdir)/libvendettaconsensus.pc.in \
...
ACLOCAL = ${SHELL} /home/devyamp/Desktop/7wuU0x-master/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
ARFLAGS = cr
AUTOCONF = ${SHELL} /home/devyamp/Desktop/7wuU0x-master/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /home/devyamp/Desktop/7wuU0x-master/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /home/devyamp/Desktop/7wuU0x-master/build-aux/missing automake-1.15
...
COPYRIGHT_HOLDERS_FINAL = The Vendetta Core developers
COPYRIGHT_HOLDERS_SUBSTITUTION = Vendetta Core
COPYRIGHT_YEAR = 2018
CPP = gcc -E
...
MAINT = 
MAKEINFO = ${SHELL} /home/devyamp/Desktop/7wuU0x-master/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL64 = 
PACKAGE = vendetta
PACKAGE_BUGREPORT = https://github.com/vendetta/vendetta/issues
PACKAGE_NAME = Vendetta Core
PACKAGE_STRING = Vendetta Core 0.17.1
PACKAGE_TARNAME = vendetta
PACKAGE_URL = https://vendettacore.org/
...
USE_QRCODE = 
USE_UPNP = 
VENDETTA_CLI_NAME = vendetta-cli
VENDETTA_DAEMON_NAME = vendettad
VENDETTA_GUI_NAME = vendetta-qt
VENDETTA_TX_NAME = vendetta-tx
VERSION = 0.17.1
...
ZMQ_CFLAGS = 
ZMQ_LIBS = 
abs_builddir = /home/devyamp/Desktop/7wuU0x-master
abs_srcdir = /home/devyamp/Desktop/7wuU0x-master
abs_top_builddir = /home/devyamp/Desktop/7wuU0x-master
abs_top_srcdir = /home/devyamp/Desktop/7wuU0x-master
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /home/devyamp/Desktop/7wuU0x-master/build-aux/install-sh
...
pkgconfig_DATA = libvendettaconsensus.pc
VENDETTAD_BIN = $(top_builddir)/src/$(VENDETTA_DAEMON_NAME)$(EXEEXT)
VENDETTA_QT_BIN = $(top_builddir)/src/qt/$(VENDETTA_GUI_NAME)$(EXEEXT)
VENDETTA_CLI_BIN = $(top_builddir)/src/$(VENDETTA_CLI_NAME)$(EXEEXT)
VENDETTA_TX_BIN = $(top_builddir)/src/$(VENDETTA_TX_NAME)$(EXEEXT)
VENDETTA_WIN_INSTALLER = $(PACKAGE)-$(PACKAGE_VERSION)-win$(WINDOWS_BITS)-setup$(EXEEXT)
empty := 
space := $(empty) $(empty)
OSX_APP = Vendetta-Qt.app
...
OSX_INSTALLER_ICONS = $(top_srcdir)/src/qt/res/icons/vendetta.icns
OSX_PLIST = $(top_builddir)/share/qt/Info.plist #not installed
OSX_QT_TRANSLATIONS = da,de,es,hu,ru,uk,zh_CN,zh_TW
DIST_DOCS = $(wildcard doc/*.md) $(wildcard doc/release-notes/*.md)
DIST_CONTRIB = $(top_srcdir)/contrib/vendetta-cli.bash-completion \
	       $(top_srcdir)/contrib/vendetta-tx.bash-completion \
	       $(top_srcdir)/contrib/vendettad.bash-completion \
...
WINDOWS_PACKAGING = $(top_srcdir)/share/pixmaps/vendetta.ico \
...
COVERAGE_INFO = baseline.info \
  test_vendetta_filtered.info total_coverage.info \
  baseline_filtered.info functional_test.info functional_test_filtered.info \
  test_vendetta_coverage.info test_vendetta.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/vendetta.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/Vendetta-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
EXTRA_DIST = $(DIST_SHARE) test/functional/test_runner.py \
	...
	test/util/vendetta-util-test.py \
	test/util/data/vendetta-util-test.json \
...
CLEANFILES = $(OSX_DMG) $(VENDETTA_WIN_INSTALLER)
...
src/config/vendetta-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/vendetta-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/vendetta-config.h
$(top_srcdir)/src/config/vendetta-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/vendetta-config.h src/config/stamp-h1
libvendettaconsensus.pc: $(top_builddir)/config.status $(srcdir)/libvendettaconsensus.pc.in
...
$(VENDETTA_WIN_INSTALLER): all-recursive
	$(MKDIR_P) $(top_builddir)/release
	STRIPPROG="$(STRIP)" $(INSTALL_STRIP_PROGRAM) $(VENDETTAD_BIN) $(top_builddir)/release
	STRIPPROG="$(STRIP)" $(INSTALL_STRIP_PROGRAM) $(VENDETTA_QT_BIN) $(top_builddir)/release
	STRIPPROG="$(STRIP)" $(INSTALL_STRIP_PROGRAM) $(VENDETTA_CLI_BIN) $(top_builddir)/release
	STRIPPROG="$(STRIP)" $(INSTALL_STRIP_PROGRAM) $(VENDETTA_TX_BIN) $(top_builddir)/release
...
$(OSX_APP)/Contents/Resources/vendetta.icns: $(OSX_INSTALLER_ICONS)
	$(MKDIR_P) $(@D)
	$(INSTALL_DATA) $< $@

$(OSX_APP)/Contents/MacOS/Vendetta-Qt: all-recursive
	$(MKDIR_P) $(@D)
	STRIPPROG="$(STRIP)" $(INSTALL_STRIP_PROGRAM)  $(VENDETTA_QT_BIN) $@
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Vendetta-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Vendetta-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#deploy: $(OSX_DMG)
#deploy: $(VENDETTA_WIN_INSTALLER)

$(VENDETTA_QT_BIN): FORCE
	$(MAKE) -C src qt/$(@F)

$(VENDETTAD_BIN): FORCE
	$(MAKE) -C src $(@F)

$(VENDETTA_CLI_BIN): FORCE
	$(MAKE) -C src $(@F)

$(VENDETTA_TX_BIN): FORCE
	$(MAKE) -C src $(@F)
...
#test_vendetta.info: baseline_filtered.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c $(LCOV_OPTS) -d $(abs_builddir)/src -t test_vendetta -o $@
#	$(LCOV) -z $(LCOV_OPTS) -d $(abs_builddir)/src

#test_vendetta_filtered.info: test_vendetta.info
#	$(abs_builddir)/contrib/filter-lcov.py $(LCOV_FILTER_PATTERN) $< $@
#	$(LCOV) -a $@ $(LCOV_OPTS) -o $@

#functional_test.info: test_vendetta_filtered.info
...
#test_vendetta_coverage.info: baseline_filtered.info test_vendetta_filtered.info
#	$(LCOV) -a $(LCOV_OPTS) baseline_filtered.info -a test_vendetta_filtered.info -o $@

#total_coverage.info: test_vendetta_filtered.info functional_test_filtered.info
#	$(LCOV) -a $(LCOV_OPTS) baseline_filtered.info -a test_vendetta_filtered.info -a functional_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_vendetta.coverage/.dirstamp:  test_vendetta_coverage.info
...
#cov: test_vendetta.coverage/.dirstamp total.coverage/.dirstamp
...
clean-local: clean-docs
	rm -rf coverage_percent.txt test_vendetta.coverage/ total.coverage/ test/tmp/ cache/ $(OSX_APP)
