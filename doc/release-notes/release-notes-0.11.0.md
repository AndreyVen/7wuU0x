Vendetta Core version 0.11.0 is now available from:

  <https://vendetta.org/bin/vendetta-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer VEN/kB fee to be
...
For example, add the following to `vendetta.conf`:
...
related to the blockchain in the vendetta system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Vendetta Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Vendetta Core will 
...
For an example script see [vendetta-submittx](https://github.com/laanwj/vendetta-submittx).
...
- #5334 `cf87536` libvendettaconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new vendetta-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only VendettaAmountField
- #5753 `0f44672` Add vendetta logo to about screen
...
- #5809 `46bfbe7` Add vendetta-cli man page
...
- #5849 `41259ca` contrib/init/vendettad.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Vendetta-Qt.app to Vendetta-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for vendetta-tx)
