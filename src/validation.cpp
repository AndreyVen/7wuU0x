...
#if defined(NDEBUG)
# error "Vendetta cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Vendetta Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept)
{
...

    {
        ...
        for (CTxMemPool::txiter it : allConflicting)
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s VEN additional fees, %d delta bytes\n",
...
void ThreadScriptCheck() {
    RenameThread("vendetta-scriptch");
...
// commitment occurs, or -1 if not found.yes
