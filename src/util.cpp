...
const char * const VENDETTA_CONF_FILENAME = "vendetta.conf";
const char * const VENDETTA_PID_FILENAME = "vendettad.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "vendettad -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Vendetta
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Vendetta
    // Mac: ~/Library/Application Support/Vendetta
    // Unix: ~/.vendetta
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Vendetta";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Vendetta";
#else
    // Unix
    return pathRet / ".vendetta";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", VENDETTA_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", VENDETTA_PID_FILENAME)));
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure Vendetta Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Vendetta Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Vendetta Core developers";
    }
